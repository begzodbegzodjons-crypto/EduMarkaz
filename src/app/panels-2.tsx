...
export function TeachersPanel() {
  ...
  function attBadge(status: string) {
    if (status === 'present') return <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">Keldi</span>
    if (status === 'absent') return <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-700 text-[9px] font-bold">Kelmadi</span>
    if (status === 'late') return <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">Kechikdi</span>
    if (status === 'sick') return <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">Kasal</span>
    if (status === 'vacation') return <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">Ta'til</span>
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="bg-card rounded-2xl border border-border/50 p-3">
                ...
                <div className="text-sm font-semibold mt-0.5 text-indigo-700">{formatMoney(t.salary_amount)}</div>
              </div>
              <div className="bg-card rounded-2xl border border-border/50 p-3">
                <div className="text-[10px] text-muted-foreground">O'qituvchi paneli</div>
                <div className="text-sm font-semibold mt-0.5">
                  {t.has_password ? (
                    <span className="text-indigo-700">✓ Parol bor</span>
                  ) : (
                    <span className="text-indigo-700">⚠ Parol yo'q</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
                <div className="text-[10px] text-indigo-700">Guruhlar</div>
                <div className="text-2xl font-bold mt-0.5 text-slate-900">{teacherGroups.length}</div>
              </div>
              <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
                <div className="text-[10px] text-indigo-700">Jami talabalar</div>
                <div className="text-2xl font-bold mt-0.5 text-slate-900">{totalStudents}</div>
              </div>
              <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
                <div className="text-[10px] text-indigo-700">Darslar (jadvalda)</div>
                <div className="text-2xl font-bold mt-0.5 text-indigo-700">{teacherSchedule.length}</div>
              </div>
              <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
                <div className="text-[10px] text-indigo-700">Ishga kelgan (30 kun)</div>
                      ...
                      {groupsWithCounts.map((g, idx) => (
                        <tr key={g.id} className="border-b border-border/20 hover:bg-muted/40">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${g.students_count >= g.max_students ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`}>
                        ...
                        return (
                          <tr key={s.id} className="border-b border-border/20 hover:bg-muted/40">
                            ...
                            <td className="px-4 py-3">
                              <span className="px-2 py-0.5 rounded bg-slate-50 text-indigo-700 text-xs font-semibold">
                  ...
                  <table className="w-full text-sm">
                      ...
                      {teacherPayouts.map((p, idx) => {
                        ...
                        return (
                          <tr key={p.id} className="border-b border-border/20 hover:bg-muted/40">
                            ...
                            <td className="px-4 py-3 text-right text-indigo-700">{p.bonus > 0 ? `+${formatMoney(p.bonus)}` : '—'}</td>
                            <td className="px-4 py-3 text-right text-indigo-700">{p.deduction > 0 ? `-${formatMoney(p.deduction)}` : '—'}</td>
                            <td className="px-4 py-3 text-right font-bold text-indigo-700">{formatMoney(total)}</td>
                            <td className="px-4 py-3 text-center">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${p.status === 'paid' ? 'bg-indigo-100 text-indigo-700' : 'bg-indigo-100 text-indigo-700'}`}>
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-border/60 bg-muted/30 font-bold">
                        ...
                        <td className="px-4 py-3 text-right text-indigo-700">{formatMoney(totalPaid + totalPending)}</td>
                        <td colSpan={2}></td>
                      </tr>
                    </tfoot>
                  </table>
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 text-xs text-indigo-700">
              ...
              O'qituvchi paneli (<code className="px-1 bg-indigo-100 rounded">/teacher</code>) orqali u o'z guruhlarining davomatini belgilaydi.
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 text-xs text-indigo-700">
        <strong>ℹ️ O'qituvchi paneli:</strong> O'qituvchilar <code className="px-1 py-0.5 bg-indigo-100 rounded">/teacher</code> manzilidan kirib, o'z guruhlaridagi o'quvchilarning davomatini belgilashlari mumkin.
                ...
                >
                  ...
                  <div className="mt-3 space-y-1.5 text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {t.has_password ? (
                        <span className="px-1.5 py-0.5 rounded-full bg-indigo-100 text-indigo-700 text-[10px] font-semibold">✓ Parol bor</span>
                      ) : (
                        <span className="px-1.5 py-0.5 rounded-full bg-indigo-100 text-indigo-700 text-[10px] font-semibold">⚠ Parol yo'q</span>
                      )}
                    </div>
                  </div>
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)} title={editing ? 'O\'qituvchini tahrirlash' : 'Yangi o\'qituvchi'} size="lg">
        <div className="space-y-3">
          ...
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 space-y-2">
            <div className="text-xs font-semibold text-indigo-700">🔐 O'qituvchi paneli uchun kirish</div>
            <div className="grid sm:grid-cols-2 gap-3">
            ...
            <p className="text-[11px] text-indigo-700">
              O'qituvchi <code className="px-1 bg-indigo-100 rounded">/teacher</code> manzilidan login (yoki telefon) va parol bilan kirib, o'z guruhlaridagi o'quvchilarning davomatini belgilay oladi.
            </p>
          </div>
...
export function GroupsPanel() {
  ...
  function statusBadge(status: string) {
    if (status === 'present') return <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">K</span>
    if (status === 'absent') return <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-700 text-[9px] font-bold">·</span>
    if (status === 'late') return <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">Kech</span>
    return <span className="text-[9px] text-muted-foreground">—</span>
  }
      ...
      <div className="space-y-5">
        ...
        {loadingDetail ? <PanelLoader /> : (
          <div className="grid lg:grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {todayStatus ? (
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                              todayStatus === 'present' ? 'bg-indigo-100 text-indigo-700' :
                            ...
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-100 text-slate-500">
                              Belgilanmagan
                            </span>
                          )}
                        </div>
              ...
              <div className="p-4 pt-0">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <button
                    ...
                    className="ml-auto px-2 py-1.5 rounded-lg bg-slate-50 hover:bg-indigo-100 text-indigo-700 text-xs"
                ...
                <div className="mt-3 pt-3 border-t border-border/40 flex items-center gap-3 text-[10px] text-muted-foreground flex-wrap">
                  <span className="flex items-center gap-1">
                    <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">K</span> Keldi
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-700 text-[9px] font-bold">·</span> Kelmadi
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[9px] font-bold">Kech</span> Kechikdi
                  </span>
                  <span className="flex items-center gap-1">
            ...
            {/* Ma'lumot */}
            <div className="lg:col-span-2 rounded-xl bg-slate-50 border border-slate-200 p-3 text-xs text-indigo-700">
            ...
            return (
              <Card key={g.id} color="sky">
                ...
                >
                  ...
                  <div className="mt-3 pt-3 border-t border-border/40 flex items-center justify-between">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${isFull ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`}>{count}/{g.max_students} talaba</span>
                    <span className="text-[10px] text-indigo-700 font-medium flex items-center gap-1">
                      Batafsil
                      <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
                    </span>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${
                    form.schedule?.startsWith('Du-Chor-Juma')
                      ? 'bg-indigo-600 text-white border-slate-200'
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="bg-card rounded-2xl border border-border/50 p-3">
            ...
            <div className="text-sm font-semibold mt-0.5 text-indigo-700">{formatMoney(c.price)}</div>
          </div>
          <div className="bg-card rounded-2xl border border-border/50 p-3">
            <div className="text-[10px] text-muted-foreground">Jami to'lovlar</div>
            <div className="text-sm font-semibold mt-0.5 text-indigo-700">{formatMoney(totalPaid)}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
            <div className="text-[10px] text-indigo-700">Guruhlar</div>
            <div className="text-2xl font-bold mt-0.5 text-indigo-700">{courseGroups.length}</div>
          </div>
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
            <div className="text-[10px] text-indigo-700">Jami talabalar</div>
            <div className="text-2xl font-bold mt-0.5 text-slate-900">{totalStudents}</div>
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-3">
            <div className="text-[10px] text-indigo-700">Faol talabalar</div>
                  ...
                  {courseGroups.map((g, idx) => (
                    <tr key={g.id} className="border-b border-border/20 hover:bg-muted/40">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">
                        ...
                        ) : <span className="text-indigo-700 text-xs">Biriktirilmagan</span>}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${g.students_count >= g.max_students ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`}>
                          {g.students_count}/{g.max_students}
                      ...
                      <tr key={s.id} className="border-b border-border/20 hover:bg-muted/40">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                            ...
                            s.status === 'paused' ? 'bg-indigo-100 text-indigo-700' :
        ...
        <Card color="violet">
          ...
          <div className="p-4 pt-0 grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-3 text-center">
              <div className="text-[10px] text-indigo-700">Jami to'langan</div>
              <div className="text-lg font-bold mt-1 text-slate-900">{formatMoney(totalPaid)}</div>
            </div>
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-3 text-center">
              <div className="text-[10px] text-indigo-700">Oylik to'lov (har talaba)</div>
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-3 text-center">
              <div className="text-[10px] text-indigo-700">Kutilayotgan oylik tushum</div>
              <div className="text-lg font-bold mt-1 text-indigo-700">{formatMoney(totalActive * Number(c.price || 0))}</div>
            </div>
          </div>
        </Card>
              ...
              <Card key={c.id} color="violet">
                ...
                >
                  ...
                  <div className="mt-3 pt-3 border-t border-border/40 grid grid-cols-3 gap-2 text-center">
                    <div><div className="text-base font-bold text-indigo-700">{groupCount}</div><div className="text-[10px] text-muted-foreground">guruh</div></div>
                    <div><div className="text-base font-bold text-indigo-700">{studentCount}</div><div className="text-[10px] text-muted-foreground">talaba</div></div>
                    <div><div className="text-base font-bold text-indigo-700">{formatMoney(c.price).replace(' so\'m', '')}</div><div className="text-[10px] text-muted-foreground">so'm</div></div>
                  </div>
            ...
            <div className="p-4 pt-0 space-y-2 max-h-96 overflow-y-auto">
              {avgByStudent.map((s, i) => (
                <div key={s.id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-muted/40">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm ${i === 0 ? 'bg-indigo-100 text-indigo-700' : i === 1 ? 'bg-indigo-100 text-indigo-700' : i === 2 ? 'bg-slate-600 text-indigo-700' : 'bg-muted text-muted-foreground'}`}>{i + 1}</div>
