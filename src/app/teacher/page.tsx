...
// =====================================================================
//  O'QITUVCHI PANELI — alohida sahifa
//  URL: /teacher
//  O'qituvchi o'z guruhlarini ko'radi va davomat belgilaydi
// =====================================================================
...
export default function TeacherPanelPage() {
  ...
  useEffect(() => {
    if (teacher) loadGroups()
  }, [teacher, loadGroups])

  // Talabalarni yuklash (guruh tanlanganda)
  useEffect(() => {
    if (!selectedGroup) {
      setStudents([])
      setAttendance({})
  ...
  // Mavjud davomatni yuklash (sana o'zgarsa)
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    const { ok, error, data } = await apiFetch('/api/teacher/attendance', {
      method: 'POST',
      body: JSON.stringify({
    ...
    setSaving(false)
    if (!ok) {
      alert(error || 'Saqlashda xatolik.')
      return
    }
    setSavedMessage(`✓ ${data?.count || records.length} ta talabaning davomati saqlandi!`)
  ...
  async function apiFetch(url: string, options?: any) {
    try {
      ...
      return { ok: false, error: e?.message || 'Network xatosi' }
    }
  }
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
            <div className="text-xs text-slate-500">Mening guruhlarim</div>
            <div className="text-2xl font-bold text-slate-800 mt-1">{groups.length}</div>
          </div>
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
            <div className="text-xs text-slate-500">Bugungi sana</div>
            <div className="text-lg font-bold text-slate-800 mt-1">
              {new Date().toLocaleDateString('uz-UZ', { day: 'numeric', month: 'long' })}
            </div>
            <div className="text-[10px] text-slate-400">{WEEKDAYS[new Date().getDay() === 0 ? 6 : new Date().getDay() - 1]}</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4">
          ...
          {loadingData ? (
            <div className="text-center py-4 text-sm text-slate-500">Yuklanmoqda...</div>
          ) : groups.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              <svg className="w-12 h-12 mx-auto mb-2 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><circle cx="9" cy="7" r="4" /><path d="M3 21v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /></svg>
              <div className="text-sm">Sizga biriktirilgan guruhlar yo'q</div>
              <div className="text-xs text-slate-400 mt-1">Admin bilan bog'laning</div>
            </div>
          ) : (
              ...
              {groups.map((g) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {g.schedule && (
                      <span className="px-2 py-0.5 rounded-full bg-blue-50 text-[10px] font-semibold text-blue-700 truncate max-w-[120px]">
                        {g.schedule}
                      </span>
                    )}
                  </div>
        ...
        {selectedGroup && selectedGroupObj && (
          ...
          >
            <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between flex-wrap gap-3">
              <div>
                <div className="font-bold text-slate-800">{selectedGroupObj.name}</div>
                <div className="text-xs text-slate-500">{selectedGroupObj.course?.name} · {students.length} o'quvchi</div>
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs text-slate-600">Dars sanasi:</label>
                <input
                  type="date"
                  value={lessonDate}
              ...
              <>
                ...
                <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-red-50 hover:bg-red-100 text-red-700 text-xs font-semibold"
                  >
                    ✗ Kelmadi
                  </button>
                  <button
                    onClick={() => setAll('late')}
                    className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 text-xs font-semibold"
                  >
                    ⏰ Kechikdi
                  </button>
                </div>
